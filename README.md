...
- Multi-currency expenses: 10 currencies (INR, USD, AED, EUR, GBP, SGD, THB, JPY, CAD, AUD); FX rate fetched server-side from Frankfurter API and locked at creation; balances always settled in the group's base currency (INR)
...
This is fully shipped. Each expense stores a `currency` (ISO 4217) and an `fx_rate_at_creation` — the rate between that currency and the group's `base_currency` (INR by default), fetched server-side from [Frankfurter API](https://www.frankfurter.app/) at the moment the expense is submitted. The netting algorithm is untouched; `computeGroupBalances` multiplies each split amount by `fx_rate_at_creation` before summing, so every contribution is expressed in the base currency before the greedy min-heap runs.

**Why the rate is locked at creation, not recomputed live:** recomputing with today's rates would cause a group's total balance to drift week to week without any new activity — members could owe different amounts for the same trip depending on when they check the app. Locking makes the ledger append-only and auditable, matching standard double-entry bookkeeping.

**Rate source:** Frankfurter API (ECB data, no key required), cached in memory for 1 hour per server process. The fetch happens inside the `addExpense` server action — never client-side — so the rate cannot be spoofed by the browser. If the API is unreachable, the expense is rejected with a clear error; it is never saved with a silently wrong rate of 1.0.

**Schema** (non-breaking — existing rows default to INR/1.0):
...
model Expense {
  ...
  // e.g. AED 600 with rate 22.68 → ₹13,608 contributed to balances
}
```
